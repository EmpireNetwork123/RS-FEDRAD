...
# Integrate Fast Gradient Sign Method (FGSM) for generating adversarial examples
def generate_adversarial_examples(model, X, y_true, epsilon=0.01):
    ...
    y_true = tf.reshape(y_true, (-1, 1))  # Now y_true will have shape (2412, 1)
    
    with tf.GradientTape() as tape:
        tape.watch(X_adv)
        y_pred = model(X_adv)
        loss = tf.keras.losses.binary_crossentropy(y_true=y_true, y_pred=y_pred)  # Correctly calculate loss with true labels
    gradients = tape.gradient(loss, X_adv)
    adv_X = X_adv + epsilon * tf.sign(gradients)
    return tf.clip_by_value(adv_X, clip_value_min=0, clip_value_max=1)  # Clip to keep pixel range valid
...
class RansomwareClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        X_train_adv = generate_adversarial_examples(self.model, self.X_train, self.y_train)
    ...
    def evaluate(self, parameters, config):
        ...
        X_test_adv = generate_adversarial_examples(self.model, self.X_test, self.y_test)
